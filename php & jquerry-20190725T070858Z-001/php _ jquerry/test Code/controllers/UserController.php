<?php 
    class UserController extends BaseController {
        ...
        */
        ...
        public function postUsersignin() {
			$userObj										=   DB::table('users')
																	->where('users.email','=',Input::get('email_login'))
																	->select('users.is_active','users.is_reset_req','users.is_mail_req','users.expired_date','users.id')
																	->first();
				...
				if(isset($userObj->is_active) && $userObj->is_active == 'Y'){
					if(isset($userObj->is_reset_req) && $userObj->is_reset_req == 'N'){
						if(isset($userObj->is_mail_req) && $userObj->is_mail_req == 'N'){
							if (Auth::attempt(array('email'=>Input::get('email_login'), 'password'=>Input::get('password_login')))) {
								return Redirect::to('censor/dashboard')->with('message', 'You are now logged in!');
																	
							} else {
								return Redirect::to('user/userregister')
									->with('error', 'username/password combination was incorrect')
									->withInput();
							}
						} else {
								return Redirect::to('user/userregister')
									->with('error', 'Your Email Confirmation not Yet. Please Check your mail')
									->withInput();
							}
					}else{
						return Redirect::to('user/userregister')
        ...
        */
        ...
        public function getChangepassword(){$this->layout                       =   View::make('layouts.admin');
			//$userArr                                                            =	Auth::user();			
			$layoutArr                                                          =	array(
                                                                                        //'userArr'					=>	$userArr
                                                                                    );
            
            $this->layout->content                                              =   View::make('user.changepassword',array('layoutArr'=>$layoutArr));
        }
        /**
    ...
    $this->layout->content							=   View::make('user.profile',array('layoutArr'=>$layoutArr));
        }
    ...
    $this->layout->content							=   View::make('user.profileinfo',array('layoutArr'=>$layoutArr));
        }
        ...
        public function getResetpassword($token = '',$id = 0){
                //$this->layout									=   View::make('layouts.resetpassword');
                $username 										=	'';
                $error											=	'';
                if($token != '' && (int)$id != 0){
    ...
    $this->layout->content							=   View::make('user.resetpassword',array('layoutArr'=>$layoutArr));			
        }
        /**
    ...
    $this->layout->content							=   View::make('user.resetlostpassword',array('layoutArr'=>$layoutArr));			
        }
            ...
            if(isset($formData['formdata']) && $formData['formdata'] != ''){
                ...
                if(isset($formDataArr['User']) && is_array($formDataArr['User']) && count($formDataArr['User']) > 0){
                    ...
                    if ($validator->fails()){
                        ...
                        $getSubscriptionObj         =   DB::table('subscriptions')
                        ...
                        $tblCustIdCnt                                       =   DB::table('users')
                                                                                 ->count();
                        $subject                                            =   "Verify your email address";
                        if($tblCustIdCnt == 0){
                            $customer_id                                    =   10000; 
                        }else{
                            $tblCustIdObj                                   =   DB::table('users')
        ...
        public function getUsermailvalidation($token = '',$id = 0){
            ...
            //echo "<pre>"; print_r($id."++".$token."++".$userId); echo "<pre>"; exit;
            if($token != '' && (int)$id != 0){
                $userDatasCnt			=   DB::table('users')
                                                        ->where('users.id','=',(int)$id)
                                                        ->where('users.remember_token','=','')
                                                        ->where('users.is_active','=','Y')
                                                        ->count();
                //echo "<pre>"; print_r($id."++".$token); echo "<pre>"; //exit;
                if($userDatasCnt == 0){
                    $userDataObj			=   DB::table('users')
                                                            ->where('users.id','=',(int)$id)
                                                            ->where('users.remember_token','=',$token)
                                                            ->first();
                   // echo "<pre>"; print_r($userDataObj); echo "<pre>"; exit;
                    if(is_object($userDataObj)){
                        $name 				=	$userDataObj->fullname;
                        $email 				=	$userDataObj->email;
                        $userCnt				=   DB::table('users')
                                                                        ->where('users.id','=',(int)$id)
                                                                        ->where('users.is_mail_req','=','Y')
                                                                        ->where('users.remember_token','=',$token)	
                                                                        ->count();
                        if($userCnt == 0){
                            $mailNotVerifyCnt++;
           ...
           //echo "<pre>"; print_r($mailNotVerifyCnt."++".$acexpiredCnt."++".$userAcntVerifyCnt); echo "<pre>"; exit;
            if($mailNotVerifyCnt == 0 && $acexpiredCnt == 0 && $userAcntVerifyCnt == 0){
                    $this->layout->content	=   View::make('user.usermailvalidation',array('layoutArr'=>$layoutArr));
            }else if($mailNotVerifyCnt == 0 && $acexpiredCnt > 0 && $userAcntVerifyCnt == 0){
                    $this->layout->content	=   View::make('user.useraccountexpired',array('layoutArr'=>$layoutArr));
            }else if($mailNotVerifyCnt == 0 && $acexpiredCnt == 0 && $userAcntVerifyCnt > 0){
                    $this->layout->content	=   View::make('user.usermailverified',array('layoutArr'=>$layoutArr));
            }
        }
